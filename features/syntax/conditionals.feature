Feature: Conditionals
  ...
  Scenario: If with block branches and else
    ...
    Then the output should be 'IfExpr { condition: Literal(Boolean(true)), then_branch: Literal(Number(Numeric { value: BigDecimal(sign=Plus, scale=0, digits=[1]) })), else_branch: Some(Literal(Number(Numeric { value: BigDecimal(sign=Plus, scale=0, digits=[2]) }))) }'

  Scenario: If with block branches and else (condition false)
    Given the input "if false { 1 } else { 2 }"
    When I parse the input
    Then the output should be 'IfExpr { condition: Literal(Boolean(false)), then_branch: Literal(Number(Numeric { value: BigDecimal(sign=Plus, scale=0, digits=[1]) })), else_branch: Some(Literal(Number(Numeric { value: BigDecimal(sign=Plus, scale=0, digits=[2]) }))) }'

  Scenario: If with block then-branch only (no else)
    Given the input "if true { 1 }"
    When I parse the input
    Then the output should be 'IfExpr { condition: Literal(Boolean(true)), then_branch: Literal(Number(Numeric { value: BigDecimal(sign=Plus, scale=0, digits=[1]) })), else_branch: None }'
  ...
  Scenario: If with parenthesized condition
    Given the input "if (true) { 1 } else { 2 }"
    When I parse the input
    Then the output should be 'IfExpr { condition: Parenthesized(Literal(Boolean(true))), then_branch: Literal(Number(Numeric { value: BigDecimal(sign=Plus, scale=0, digits=[1]) })), else_branch: Some(Literal(Number(Numeric { value: BigDecimal(sign=Plus, scale=0, digits=[2]) }))) }'
